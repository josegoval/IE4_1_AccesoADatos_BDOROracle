...
public class InsertFunctions {
	...
	public static boolean createAlumno(Alumno alumno) {
		...
		if (con.validarConexion()) {
			try {
				// Se prepara
				CallableStatement cstmt = con.getConexion()
	...
	public static boolean matricularAsignaturas(String dni, int cod_asig) {
		...
		if (con.validarConexion()) {
			try {
				// Se prepara
				CallableStatement cstmt = con.getConexion().prepareCall("{? = CALL MATRICULAR_ASIGNATURA(?,?)}");
				cstmt.registerOutParameter(1, Types.INTEGER);
	...
	public static boolean createAsignatura(Asignatura asignatura) {
		...
		if (con.validarConexion()) {
			try {
				// Se prepara
				CallableStatement cstmt = con.getConexion().prepareCall("{? = CALL CREATE_ASIGNATURA(?,?,?)}");
				cstmt.registerOutParameter(1, Types.INTEGER);
	...
	public static boolean createCurso(Curso curso) {
		...
		if (con.validarConexion()) {
			try {
				// Se prepara
				CallableStatement cstmt = con.getConexion().prepareCall("{? = CALL CREATE_CURSO(?,?,?,?)}");
				cstmt.registerOutParameter(1, Types.INTEGER);
